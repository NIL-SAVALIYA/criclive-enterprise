          ...
          <div className="glass-panel w-full max-w-5xl rounded-3xl border border-emerald-500/40 p-6 space-y-6 shadow-2xl my-auto">
            ...
            <div className="flex flex-wrap gap-2 border-b border-gray-800/80 pb-2 text-xs font-bold">
              {[
                ...
                { id: 'scorers', label: 'Match Scorers', icon: UserCheck },
                ...
                {manageTab === 'scorers' && (
                  <div className="space-y-4">
                    <div className="glass-panel p-4 rounded-2xl border border-gray-800">
                      ...
                      <p className="text-[11px] text-gray-400 mt-0.5">
                        Assign official certified Scorer accounts to each fixture. Scorers can access the live
                        scoring console.
                      </p>
                    </div>
                    ...
                    <div className="space-y-3">
                      {manageDashboard?.matches?.length === 0 ? (
                        ...
                        manageDashboard.matches.map((m) => (
                          <div
                            key={m.id}
                            className="glass-panel p-4 rounded-xl border border-gray-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-xs"
                          >
                            <div>
                              ...
                              <p className="text-[11px] text-gray-400 font-mono">
                                {new Date(m.matchDate).toLocaleDateString()} | {m.venue} | Status:{' '}
                            ...
                            <div className="flex items-center gap-2 w-full md:w-auto">
                              <select
                                value={m.scorerId || ''}
                                onChange={(e) => handleAssignMatchScorer(m.id, e.target.value)}
                                disabled={actionLoading || m.status === 'COMPLETED'}
                                className="bg-gray-900 border border-gray-700 text-white text-xs rounded-xl p-2 flex-1 md:w-56"
                              >
                                <option value="">-- No Scorer Assigned --</option>
                                {manageDashboard?.eligibleScorers?.map((sc) => (
                                  <option key={sc.id} value={sc.id}>
                                    {sc.firstName} {sc.lastName} ({sc.email})
                                  </option>
                                ))}
                              </select>
                              ...
                              {m.scorerId && (
                                <Link
                                  to={`/admin/scoring/${m.id}`}
                                  target="_blank"
                                  className="px-2.5 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl flex items-center gap-1 font-bold text-[11px]"
                                  title="Open Scorer Console in new tab"
                                >
